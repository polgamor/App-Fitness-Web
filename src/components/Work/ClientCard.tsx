...
import { Cliente, useAuth } from "../../context/AuthContext"; // Importa useAuth
...
export default function ClientCard({ client, onClick }: ClientCardProps) {
  ...
  const styles: {
    ...
    card: {
      cursor: isEntrenadorActivo ? (isActive ? "pointer" : "not-allowed") : "not-allowed", // Controla el cursor según el entrenador
      padding: "1.25rem",
      borderRadius: "0.5rem",
      ...
      maxWidth: "300px",
      height: "12rem",
      opacity: isEntrenadorActivo ? (isActive ? 1 : 0.5) : 0.6,
      position: "relative",
      overflow: "hidden",
    },
    inactiveOverlay: {
  ...
  const handleMouseEnter = (e: React.MouseEvent<HTMLDivElement>) => {
    if (!isActive || !isEntrenadorActivo) return; 
  ...
  const handleClick = () => {
    if (isActive && isEntrenadorActivo) { 
      onClick();
    }
  };
